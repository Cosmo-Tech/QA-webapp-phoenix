...
import { SCENARIO_EXAMPLE } from '../stubbing/default';
...
const startInterceptionMiddlewares = () => {
  cy.intercept({ url: API_REGEX.ALL, middleware: true }, (req) => {
    ...
    if (stub.isEnabledFor('GET_WORKSPACES') && stub.getActualWorkspaceId() !== null && stub.getFakeWorkspaceId() !== null) {
...
const interceptRunScenario = (scenarioRunResponse = {}) => {
  ...
  cy.intercept({ method: 'POST', url: API_REGEX.RUN_SCENARIO, times: 1 }, (req) => {
    if (stub.isEnabledFor('RUN_SCENARIO')) {
      req.reply(scenarioRunResponse);
    }
  }).as(alias);
  return alias;
};

const interceptUpdateScenario = (scenarioId, validateRequest) => {
...
const interceptGetScenario = (scenarioId) => {
  ...
  if (scenarioId) {
    interceptionURL = new RegExp('^' + API_ENDPOINT.WORKSPACES + '/((w|W)-[\\w]+)/scenarios/(' + scenarioId + ')' + '$');
  }
